...
export function createHwarCoreConfigFromEnv(env: NodeJS.ProcessEnv = process.env): HwarCoreConfig {
    return {
...
function createDisabledAnalysis(deps: HwarDeps) {
    ...
    return {
        enqueueForVideos: async (): Promise<DisabledStatus> => {
            logger?.warn?.('[analysis.enqueueForVideos] disabled – subsystem disabled by config');
            return { status: 'disabled' };
        },
        runTick: async (): Promise<DisabledTickResult> => {
            logger?.debug?.('[analysis.runTick] no-op – subsystem disabled by config');
...
function createDisabledKeyframes(deps: HwarDeps) {
    ...
    return {
        enqueueForProject: async (): Promise<DisabledStatus> => {
            logger?.warn?.('[keyframes.enqueueForProject] disabled – subsystem disabled by config');
            return { status: 'disabled' };
        },
        runTick: async (): Promise<DisabledTickResult> => {
            logger?.debug?.('[keyframes.runTick] no-op – subsystem disabled by config');
            return { status: 'disabled', processed: 0 };
        },
        getJobStatus: async (): Promise<DisabledStatus> => {
            return { status: 'disabled' };
...
function createDisabledGeneration(deps: HwarDeps) {
    ...
    return {
        createProjectFromAnalysis: async (): Promise<DisabledStatus> => {
            logger?.warn?.('[generation.createProjectFromAnalysis] disabled – subsystem NYI');
            return { status: 'disabled' };
        },
        runTick: async (): Promise<DisabledTickResult> => {
