...
const candidates: any[] = [
  {
    ...
    scenes: [{ phase: "HOOK", duration: 8, description: "Scene" }],
  },
  {
    title: "Second",
...
function renderPanel(
  approve = vi.fn().mockResolvedValue({
    revision: { revisionNumber: 1, sourceCandidateIndex: 0 },
  }),
) {
  render(
...
describe("ScenarioCandidateApprovalPanel", () => {
  ...
  it("submits only artifact and candidate index, preserves its key across retry, and shows success", async () => {
    ...
    const approve = vi
      ...
      .mockRejectedValueOnce(
        Object.assign(new Error("stack secret"), {
          code: "IDEMPOTENCY_KEY_REUSED",
        }),
      )
      .mockResolvedValueOnce({
        revision: { revisionNumber: 2, sourceCandidateIndex: 0 },
      });
    renderPanel(approve);
    ...
    expect(screen.queryByText(/stack secret/i)).not.toBeInTheDocument();
    await user.click(screen.getByRole("button", { name: /approve selected/i }));
    ...
    expect(approve).toHaveBeenNthCalledWith(
      1,
      {
        scenarioArtifactId: "11111111-1111-4111-8111-111111111111",
  ...
  it("can approve a later candidate as a new revision", async () => {
    ...
    const approve = vi
      ...
      .mockResolvedValueOnce({
        revision: { revisionNumber: 2, sourceCandidateIndex: 1 },
      });
    renderPanel(approve);
    await user.click(screen.getByText("First"));
    await user.click(screen.getByRole("button", { name: /approve selected/i }));
    await screen.findByText(/Approved revision 1/);
    await user.click(screen.getByText("Second"));
    await user.click(screen.getByRole("button", { name: /approve as new/i }));
    expect(await screen.findByText(/Approved revision 2/)).toBeInTheDocument();
    expect(approve).toHaveBeenLastCalledWith(
      expect.objectContaining({ sourceCandidateIndex: 1 }),
      expect.any(String),
    );
  });
});
