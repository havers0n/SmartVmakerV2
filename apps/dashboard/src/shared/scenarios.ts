...
// Keep required fields aligned with generate_video_scenarios in
// src/app/api/actions/handlers/generation.ts until Zod becomes the JSON Schema source.
export const sceneSchema = z
  .object({
    ...
    cameraCommands: z.array(nonBlankString('Camera command', 500)).max(20).optional(),
  })
  .strict();
...
const errorMessages: Record<ScenarioGenerationErrorCode, string> = {
  SCENARIO_GENERATION_INVALID_TYPE: 'The model returned scenarios in an unsupported format.',
  SCENARIO_GENERATION_JSON_PARSE_FAILED: 'The model returned malformed scenario data.',
  SCENARIO_GENERATION_SCHEMA_VALIDATION_FAILED: 'The generated scenarios are missing required data.',
  SCENARIO_GENERATION_EMPTY: 'The model did not generate any scenarios.',
  SCENARIO_GENERATION_TRUNCATED: 'The model response was incomplete. Please retry scenario generation.',
};
...
export function isTruncationFinishReason(finishReason: string | null | undefined): boolean {
  if (!finishReason) return false;
  const normalized = finishReason.toLowerCase().replace(/[\s-]+/g, '_');
  return normalized === 'length' || normalized === 'max_tokens' || normalized === 'token_limit';
}
...
export function normalizeAndValidateScenarios(
  ...
  if (isTruncationFinishReason(finishReason)) {
    throw new ScenarioGenerationError('SCENARIO_GENERATION_TRUNCATED', { finishReason });
  }
  ...
  if (typeof value === 'string') {
    ...
    try {
      ...
      const code = looksLikeTruncatedJson(rawString, error)
        ? 'SCENARIO_GENERATION_TRUNCATED'
        : 'SCENARIO_GENERATION_JSON_PARSE_FAILED';
      throw new ScenarioGenerationError(code, {
...
export function parseStoredScenarios(rawValue: unknown):
  | { success: true; scenarios: Scenario[] }
  | { success: false; code: ScenarioGenerationErrorCode } {
  try {
    return { success: true, scenarios: normalizeAndValidateScenarios(rawValue) };
  } catch (error) {
    if (error instanceof ScenarioGenerationError) {
      return { success: false, code: error.code };
    }
    return { success: false, code: 'SCENARIO_GENERATION_SCHEMA_VALIDATION_FAILED' };
  }
}
