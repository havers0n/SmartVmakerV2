...
import { createTextClient, generateScenariosWithTools, type TextGenerationResponse } from '@scrimspec/halu-client';
...
async function generateScenariosWithMiniMax(input: {
  ...
  if (!apiKey) {
    ...
    ], finishReason: 'stop', rawResponseLength: 0, toolName: 'development_mock' };
  }
  ...
  try {
    ...
    const tools: ChatCompletionTool[] = [
      ...
      },
      {
        type: 'function',
        function: {
          // Keep required fields aligned with @/shared/scenarios until Zod is the JSON Schema source.
    ...
    if (response.toolCalls && response.toolCalls.length > 0) {
      ...
      for (const toolCall of response.toolCalls) {
        if (toolCall.function.name === 'get_story_preset_details') {
          ...
          return extractGeneratedScenarioPayload(response, toolCall);
        }
      }
...
${toolResults.map(tr => JSON.stringify(tr.result, null, 2)).join('\n\n')}`;
        ...
        if (finalResponse.toolCalls) {
          ...
          if (generateCall) {
            return extractGeneratedScenarioPayload(finalResponse, generateCall);
          }
        }
      }
    }
    ...
    if (error instanceof ScenarioGenerationError) throw error;
    if (!isExplicitDevelopmentMockGeneration()) {
...
interface GeneratedScenarioPayload {
  ...
  toolName: string;
  rawScenarioPayloadLength?: number;
  toolArgumentsParseFailed?: boolean;
}

const scenarioToolArgumentsSchema = z.object({ scenarios: z.unknown() }).passthrough();

function extractGeneratedScenarioPayload(
  response: TextGenerationResponse,
  toolCall: NonNullable<TextGenerationResponse['toolCalls']>[number],
): GeneratedScenarioPayload {
  const parsed = scenarioToolArgumentsSchema.safeParse(toolCall.function.argumentsParsed);
  if (!parsed.success) {
    return {
      rawValue: undefined,
      finishReason: response.finishReason,
      rawResponseLength: toolCall.function.arguments.length,
      toolName: toolCall.function.name,
      toolArgumentsParseFailed: true,
    };
  }
  return {
    rawValue: parsed.data.scenarios,
    finishReason: response.finishReason,
    rawResponseLength: toolCall.function.arguments.length,
    toolName: toolCall.function.name,
    rawScenarioPayloadLength:
      typeof parsed.data.scenarios === 'string' ? parsed.data.scenarios.length : undefined,
  };
}
...
export async function startProject(
  ...
  logger.info(
    { source: validated.source, ratio: validated.ratio, correlationId },
  ...
  const created = await createProjectWithSnapshot({
    ...
    generateScenarios: async (context) => {
      const generated = await generateScenariosWithMiniMax({
        source: validated.source,
        prompt: validated.prompt ?? (validated.source === 'trends' ? getTrends().find((trend) => trend.id === validated.trendId)?.title ?? validated.trendId : ''),
        contentFormat: context.contentFormat,
        storyTemplate: context.storyTemplate,
        ratio: validated.ratio,
        lang: validated.lang,
      });
      try {
        if (generated.toolArgumentsParseFailed && !isTruncationFinishReason(generated.finishReason)) {
          throw new ScenarioGenerationError('SCENARIO_GENERATION_JSON_PARSE_FAILED', {
            rawPayloadLength: generated.rawResponseLength,
          });
        }
        return normalizeAndValidateScenarios(generated.rawValue, generated.finishReason);
      } catch (error) {
        logger.error({
          provider: 'minimax',
          model: textModelId ?? 'MiniMax-M2',
          finishReason: generated.finishReason,
          rawResponseLength: generated.rawResponseLength,
          rawScenarioPayloadLength: generated.rawScenarioPayloadLength,
          toolName: generated.toolName,
          correlationId,
          errorCode: error instanceof ScenarioGenerationError ? error.code : 'UNKNOWN',
          validationIssueCategories:
            error instanceof ScenarioGenerationError ? error.issueCategories : undefined,
        }, 'Scenario generation result rejected before project persistence');
        throw error;
      }
    },
  });
