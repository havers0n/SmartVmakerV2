...
import {
  ...
  ScenarioApprovalError,
} from "@/server/scenario-approval";
...
export async function POST(
  ...
  try {
    const result = await approveScenarioCandidate(
      userId,
      params.project_id,
      params.run_id,
      await request.json(),
    ...
    if (
      error instanceof ScenarioApprovalError ||
      error instanceof GenerationFoundationError
    )
      return NextResponse.json(
        {
          code:
