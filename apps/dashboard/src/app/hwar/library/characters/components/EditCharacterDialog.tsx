...
import { updateCharacter } from "@/shared/api/actions";
...
export function EditCharacterDialog({
  ...
  const form = useForm<FormValues>({
    ...
    defaultValues: {
      ...
      basePrompt: (character.stylePresets?.base_prompt as string) || "",
      negativePrompt: (character.stylePresets?.negative_prompt as string) || "",
      referenceImageUrls: character.referenceImageUrls?.join("\n") || "",
    },
  });
  ...
  const updateMutation = useMutation({
    mutationFn: (values: any) => updateCharacter(character.id, values),
    onSuccess: () => {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Update the character profile, reference images, and style presets
          </DialogDescription>
        </DialogHeader>
